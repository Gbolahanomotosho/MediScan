# database.py - MediScan Database Schema (FIXED)
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Text, Boolean, JSON, LargeBinary, ForeignKey
...
class Diagnosis(Base):
    ...
    # FIXED: Added proper foreign key columns
...
class Finding(Base):
    ...
    # FIXED: Added proper foreign key column
...
class AuditLog(Base):
    ...
    # Context - FIXED: Added proper foreign key columns
    diagnosis_id = Column(Integer, ForeignKey('diagnoses.id'), nullable=True, index=True)
...
def log_activity(db: SessionLocal, activity_type: str, description: str, **kwargs):
    """Log system activity"""
    try:
        # Handle the extra_data parameter properly
        extra_data = kwargs.pop('additional_data', None)  # Remove if exists
...
def record_metric(db: SessionLocal, metric_name: str, value: float, metric_type: str, **kwargs):
    ...
    try:
        # Handle the metadata parameter properly
        metadata_value = kwargs.pop('metadata', None)
        if metadata_value is not None:
            kwargs['additional_metadata'] = metadata_value
