#!/usr/bin/env python3
# analyze.py - Exact CheXNet Compatible X-ray Analysis Engine
import os
import sys
import torch
...
class XRayAnalyzer:
    ...
    def _register_hooks(self):
        ...
        try:
            # Access through DataParallel wrapper
    ...
    def analyze_xray(
        ...
        try:
            ...
            with torch.no_grad():
                # Handle TenCrop format exactly like original
                batch_size, n_crops, c, h, w = crops_tensor.size()
                
                # Reshape for model input
                input_var = crops_tensor.view(-1, c, h, w)
                ...
                probabilities = output_mean.cpu().numpy()[0]  # Take first batch
