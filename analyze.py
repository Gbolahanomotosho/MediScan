#!/usr/bin/env python3
# analyze.py - Fixed CheXNet Compatible X-ray Analysis Engine with Working Grad-CAM
import os
import sys
import torch
...
class XRayAnalyzer:
    """Production X-ray analyzer - Exact CheXNet compatibility with FIXED Grad-CAM"""
    ...
    def __init__(self, model_path: str = "model.pth.tar", device: str = "cpu"):
        ...
        self.target_layer = None  # Store target layer reference
    ...
    def _register_hooks(self):
        """FIXED: Register forward and backward hooks for Grad-CAM"""
        def forward_hook(module, input, output):
            self.features = output
            logger.debug(f"Forward hook activated, features shape: {output.shape}")

        def backward_hook(module, grad_input, grad_output):
            if grad_output[0] is not None:
                self.gradients = grad_output[0]
                logger.debug(f"Backward hook activated, gradients shape: {grad_output[0].shape}")

        try:
            # FIXED: Find the correct target layer for Grad-CAM
            # For DenseNet121, we want the last convolutional layer before classification
            target_layer = None
            
            # Method 1: Try to find the last conv layer in features
            try:
                # DenseNet structure: features.denseblock4.denselayer16.conv2
                features_module = self.model.module.densenet121.features
                target_layer = features_module.denseblock4.denselayer16.conv2
                logger.info("Found target layer: denseblock4.denselayer16.conv2")
            except AttributeError:
                logger.warning("Standard DenseNet layer path not found, searching for alternatives...")
                
                # Method 2: Search through all modules to find the last conv2d
                conv_layers = []
                ...
                if conv_layers:
                    ...
                    logger.info(f"Using last conv layer: {layer_name}")
                else:
                    raise AttributeError("No convolutional layers found")
            ...
            logger.error(f"❌ Could not register Grad-CAM hooks: {e}")
            logger.warning("Grad-CAM visualization will not work properly")
            # Continue without Grad-CAM rather than failing completely
    ...
    def _generate_gradcam(self, image: Image.Image, class_idx: int) -> np.ndarray:
        """FIXED: Generate Grad-CAM heatmap for CheXNet"""
        try:
            if self.target_layer is None:
                logger.warning("No target layer available for Grad-CAM")
            ...
            # Ensure we have features from forward hook
            if self.features is None:
                logger.error("No features captured in forward hook")
                return np.zeros((224, 224))
            ...
            logger.debug(f"Class {class_idx} score: {class_score.item()}")
            ...
            if self.gradients is None:
                logger.error("No gradients captured in backward hook")
                return np.zeros((224, 224))
            ...
            # Generate CAM (weighted combination of feature maps)
            ...
            if grad_cam.numel() > 0:
                ...
                logger.info(f"✅ Grad-CAM generated successfully, shape: {grad_cam.shape}, range: [{grad_cam.min():.3f}, {grad_cam.max():.3f}]")
            else:
                logger.warning("Empty grad_cam tensor")
    ...
    def _create_heatmap_overlay(self, image: Image.Image, heatmap: np.ndarray, findings: List[Dict]) -> Image.Image:
        """FIXED: Create heatmap overlay for medical visualization"""
        try:
            ...
            # Using COLORMAP_JET for better visibility instead of HOT
            heatmap_colored = cv2.applyColorMap(
                np.uint8(255 * heatmap_normalized), cv2.COLORMAP_JET
            )
            heatmap_colored = cv2.cvtColor(heatmap_colored, cv2.COLOR_BGR2RGB)
            
            # Create a mask to only show significant heatmap areas (threshold > 0.3)
            mask = heatmap_normalized > 0.3
            overlay = image_np.copy()
            overlay[mask] = cv2.addWeighted(
                image_np, 0.6, heatmap_colored, 0.4, 0
            )[mask]
            
            logger.info(f"✅ Heatmap overlay created with {np.sum(mask)} active pixels")
    ...
    def analyze_xray(
        ...
        """
        Analyze chest X-ray image using exact CheXNet model with FIXED heatmap generation
        """
        ...
        try:
            ...
            with torch.no_grad():
                # Handle TenCrop format exactly like original model.py
            ...
            if generate_heatmap and self.target_layer is not None:
                try:
                    if findings:
                        ...
                        logger.info(f"Generating Grad-CAM for class {top_finding_idx} ({DISEASE_LABELS[top_finding_idx]})")
                    else:
                        # Use the class with highest probability even if below threshold
                        top_finding_idx = np.argmax(probabilities)
                        logger.info(f"No findings above threshold, using highest probability class {top_finding_idx}")
                    
                    heatmap = self._generate_gradcam(original_image, top_finding_idx)
                    
                    # Create overlay if heatmap generation was successful
                    if np.any(heatmap > 0):
                        ...
                        logger.info("✅ Heatmap successfully generated and saved")
                    else:
                        logger.warning("Heatmap generation produced empty result, using original image")
                ...
                if self.target_layer is None:
                    logger.warning("No target layer available for heatmap generation")
            ...
            if heatmap_bytes is None:
                ...
                heatmap_bytes = heatmap_buffer.getvalue()
                logger.info("Using original image as heatmap was not generated")
            ...
            metadata = {
                ...
                'heatmap_generated': self.target_layer is not None and generate_heatmap
            }
    ...
    def _generate_report(self, findings: List[Dict], metadata: Dict) -> List[str]:
        ...
        if not findings or metadata.get('has_abnormalities', False) is False:
            ...
            if urgent_findings:
                ...
                for finding in urgent_findings:
                    report.append(
                        f"  • {finding['condition']}: {finding['confidence']:.3f} probability - {finding['description']}"
                    )
                report.append("")
        ...
        if metadata.get('heatmap_generated', False):
            report.append("Grad-CAM Heatmap: Generated successfully")
        else:
            report.append("Grad-CAM Heatmap: Not available")
        
        return report

# Keep all the existing demo mode functions unchanged...
def _generate_demo_heatmap(original_image: Image.Image) -> bytes:
    """Generate a synthetic heatmap overlay for demo purposes"""
    try:
        # Resize image to standard size
        img = original_image.resize((224, 224)).convert('RGB')
        img_array = np.array(img)
        
        # Create a synthetic heatmap with realistic medical patterns
        height, width = 224, 224
        
        # Generate multiple "hotspots" that might indicate pathology
        heatmap = np.zeros((height, width), dtype=np.float32)
        
        # Add some realistic hotspots (simulating areas of interest)
        # Lower right lung area (consolidation)
        y1, x1 = 120, 140
        for i in range(40):
            for j in range(35):
        ...
        logger.info("Demo heatmap generated successfully")
...
def _demo_analysis(
    ...
    """Demo mode analysis with synthetic heatmap generation"""
    start_time = time.time()
    logger.info("Running DEMO mode analysis with synthetic results")
    
    try:
        # Load and process the original image
        original_image = Image.open(io.BytesIO(image_bytes)).convert('RGB')
        
        # Generate demo findings (sample medical conditions)
        demo_conditions = [
        ...
        logger.info(f"Demo analysis completed in {processing_time:.2f}s with {len(findings)} findings")
        return report, heatmap_bytes, metadata
        ...
        error_report = [f"Demo analysis failed: {str(e)}"]
        metadata = {
            'processing_time': processing_time,
            'error': str(e),
            'mode': 'demo'
...
def _generate_demo_report(findings: List[Dict], metadata: Dict) -> List[str]:
    """Generate demo report with medical findings - COMPATIBLE WITH APP.PY"""
    report = []
    
    # Add demo mode indicator
    report.append("[DEMO MODE] This is a demonstration analysis with sample results.")
    report.append("")
    
    if not findings or not metadata.get('has_abnormalities', False):
        report.append(
            "No significant abnormalities detected in this demonstration analysis. "
            "This is a sample result for testing purposes only."
        )
    else:
        report.append("Demo CheXNet Analysis Results:")
        report.append("")
        ...
        if urgent_findings:
            report.append("🔍 SIGNIFICANT FINDINGS - Medical consultation recommended:")
            for finding in urgent_findings:
                # FORMAT FOR APP.PY COMPATIBILITY: "condition: confidence probability - description"
                report.append(
                    f"  • {finding['condition']}: {finding['confidence']:.3f} probability - {finding['description']}"
                )
            report.append("")
        ...
        # Add demo disclaimer
        report.append(
            "DEMO DISCLAIMER: These are simulated results for demonstration purposes only. "
            "This system requires a trained AI model for real medical analysis. "
            "Do not use these results for any medical decisions."
        )
    
    # Add technical information
    report.append(f"Image Quality Score: {metadata.get('image_quality_score', 0):.2f}/1.00")
    report.append(f"Processing Time: {metadata.get('processing_time', 0):.2f} seconds")
    report.append("Model: Demo Mode (No trained model available)")
...
def analyze_xray(
    ...
    """
    Main analysis function with demo mode support
    """
    model_available = check_model_availability()
    
    if not model_available:
        # DEMO MODE - Generate sample results with heatmap
        logger.info("Running in DEMO mode - no trained model available")
        return _demo_analysis(image_bytes, threshold, generate_heatmap)
    
    try:
        # PRODUCTION MODE - Use real model
        logger.info("Running in PRODUCTION mode with trained model")
...
if __name__ == "__main__":
    ...
    if check_model_availability():
        ...
        print("Upload your model.pth.tar file to enable production analysis")
