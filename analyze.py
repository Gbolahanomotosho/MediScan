#!/usr/bin/env python3
# analyze.py - Complete CheXNet Compatible X-ray Analysis Engine with Demo Mode
import os
import sys
import torch
    ...
    def _register_hooks(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    # Fixed: Use getattr or indexing instead of invalid .2 syntax
                    denseblock4 = self.model.module.densenet121.features.denseblock4
                    denselayer16 = denseblock4.denselayer16
                    # Try to get conv2 or the second conv layer
                    if hasattr(denselayer16, 'conv2'):
                        target_layer = denselayer16.conv2
                    elif hasattr(denselayer16, 'conv'):
                        # If conv is a sequential, get the second layer
                        conv_layers = denselayer16.conv
                        if hasattr(conv_layers, '__getitem__') and len(conv_layers) > 2:
                            target_layer = conv_layers[2]  # Use indexing instead of .2
                        else:
                            target_layer = conv_layers
                    else:
                        raise AttributeError("Could not find conv2 layer")
                except AttributeError:
