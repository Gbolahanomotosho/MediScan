# app.py - Updated MediScan FastAPI Application with Demo Mode
...
async def analyze_xray_endpoint(
    ...
    """
    Analyze uploaded chest X-ray image (supports both production and demo modes)
    """
    ...
    try:
        ...
        # Perform AI analysis (works in both production and demo mode)
        ...
        try:
            ...
            return HTMLResponse(f"""
            <html>
            <head><title>MediScan AI - Analysis Results</title></head>
            <body style="font-family: Arial, sans-serif; max-width: 800px; margin: 50px auto; padding: 20px;">
                <h1>MediScan AI - Analysis Results</h1>
                {'<div style="background: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0;"><strong>Demo Mode:</strong> These are sample results for demonstration purposes.</div>' if is_demo_mode else ''}
                ...
                {'<div style="margin: 20px 0;"><img src="' + f'/static/{report_id}_heatmap.png' + '" alt="Analysis Heatmap" style="max-width: 400px;"></div>' if heatmap_path else ''}
