# app.py - Optimized MediScan FastAPI Application for Render.com
...
async def startup_event():
    ...
    logger.info("Starting MediScan AI Application v2.0.0 (Render.com Optimized)")
    
    # Check model availability
    model_info = get_model_info()
    if model_info['mode'] == 'demo':
        logger.warning("⚠️  Running in DEMO MODE - No trained model available")
        logger.warning("    This will show sample results for demonstration only")
    else:
...
async def home_page(request: Request):
    ...
    try:
        ...
        <body style="font-family: Arial, sans-serif; max-width: 800px; margin: 50px auto; padding: 20px;">
            <h1>MediScan AI - Demo Mode</h1>
            <div style="background: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; border-radius: 5px; margin: 20px 0;">
                <p><strong>Demo Mode Active:</strong> No trained AI model detected.</p>
                <p>This system will generate sample analysis results for demonstration purposes.</p>
            </div>
            <form action="/analyze/" method="post" enctype="multipart/form-data" style="margin: 20px 0;">
                ...
                <div style="margin: 15px 0;">
                    <label>X-ray Image: <input type="file" name="file" accept="image/*" required style="margin-left: 10px; padding: 5px;"></label>
                </div>
...
async def analyze_xray_endpoint(
    ...
    """
    Analyze uploaded chest X-ray image (optimized for Render.com)
    """
    ...
    try:
        ...
        if memory.percent > 85:  # If memory usage > 85%
            ...
            # If still high after cleanup, return error
            memory = psutil.virtual_memory()
        ...
        log_activity(
            db, "analysis_started", 
            f"X-ray analysis initiated for patient {patient_name} in {model_info['mode']} mode",
            user_type="doctor",
        ...
        logger.info(f"File validation passed for {file.filename} ({model_info['mode']} mode)")
        ...
        diagnosis_data = {
            ...
            'model_version': model_info.get('model_architecture', 'Unknown') + (' (Demo)' if is_demo_mode else '')
        }
        ...
        # Perform AI analysis with memory monitoring
        logger.info(f"Starting {model_info['mode']} AI analysis...")
        ...
        del contents  # Free the large image data
        gc.collect()
        ...
        logger.info(f"{model_info['mode']} analysis completed in {analysis_time:.2f}s")
        ...
        # Generate PDF report in background (for email)
        ...
        log_activity(
            ...
            f"{model_info['mode']} X-ray analysis completed for {patient_name}. Found {len(findings_data)} findings.",
            user_type="doctor",
            user_identifier=doctor_name,
        ...
        # Prepare response data with demo mode indication
        response_data = {
            "request": request,
            "report_id": report_id,
        ...
        try:
            ...
            <head>
                ...
                <style>
                    ...
                    .demo-banner {{ background: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; border: 1px solid #ffeaa7; }}
                    .error-safe {{ color: #666; font-style: italic; }}
                </style>
            </head>
            <body>
                <h1>MediScan AI - Analysis Results</h1>
                {'<div class="demo-banner"><strong>Demo Mode:</strong> These are sample results for demonstration purposes.</div>' if is_demo_mode else ''}
                ...
                {'<div style="margin: 20px 0;"><img src="' + f'/static/{report_id}_heatmap.png' + '" alt="Analysis Heatmap" style="max-width: 400px; border: 1px solid #ccc;"></div>' if heatmap_path else ''}
                ...
                <div class="error-safe">
                    <p><small>If this page doesn't display properly, your report is still being generated and will be emailed to you.</small></p>
                </div>
            </body>
            </html>
            """)
        ...
        if "502" in str(e) or "Bad Gateway" in str(e):
            ...
            raise HTTPException(status_code=500, detail="Analysis failed. Please try again or contact support if the problem persists.")
...
async def generate_and_send_report(
    ...
    try:
        ...
        # Send email (skip in demo mode to avoid sending fake results)
        if not is_demo_mode:
            try:
